/* memsize.c - MemTest-86  Version 4.1
 *
...
void mem_size(void)
{
	...
	btrace(0, __LINE__, "MemS Start", 1, 0, 0);
	...
	if (e820_nr == 0 && alt_mem_k == 0 && ext_mem_k == 0) {
		...
		// btrace(0, __LINE__, "E820 NR   ", 1, e820_nr, 0);
		for (i=0; i< mem_info.e820_nr; i++) {
			...
			// btrace(0, __LINE__, "E820 ADDR ", 0, (long)e820[i].addr, (long)(e820[i].addr + e820[i].size));
			// btrace(0, __LINE__, "E820 TYPE ", 1, e820[i].type, 0);
		}
	}
	memsize_820();
	...
	adj_mem();
	btrace(0, __LINE__, "MemS End  ", 1, 0, 0);
}
...
static void memsize_linuxbios(void)
{
	...
	btrace(0, __LINE__, "MemS Linux", 1, 0, 0);
...
static void memsize_820()
{
	...
	btrace(0, __LINE__, "MemS e820 ", 1, nr, 0);
	...
	for (i=0; i<nr; i++) {
		if (nm[i].type == E820_RAM || nm[i].type == E820_ACPI) {
			...
			if (start > RES_START && start < RES_END) {
				btrace(0, __LINE__, "e820 start", 1, (long)(start >> 12), (long)(end >> 12));
				if (end < RES_END) {
					continue;
				}
				start = RES_END;
			...
			if (end > RES_START && end < RES_END) {
				btrace(0, __LINE__, "e820 end  ", 1, (long)(start >> 12), (long)(end >> 12));
				end = RES_START;
			}
			...
			// btrace(0, __LINE__, "e820 FENT ", 1, v->pmap[n].start, v->pmap[n].end);
			n++;
		}
	}
	btrace(0, __LINE__, "e820 segs ", 1, n, 0);
	v->msegs = n;
}
...
static void memsize_801(void)
{
	...
	if (alt_mem_k < ext_mem_k) {
		btrace(0, __LINE__, "MemS e88  ", 1, ext_mem_k, 0);
		mem_size = ext_mem_k;
	} else {
		btrace(0, __LINE__, "MemS e801 ", 1, alt_mem_k, 0);
		mem_size = alt_mem_k;
	}
	...
	short old_nr)
{
	...
	*/
	...
	for (i=0; i<old_nr; i++) {
		...
		{
			btrace(0, __LINE__, "e820 err0 ", 1, (long)(biosmap[i].addr >> 12), (long)(biosmap[i].size >> 12));
			return 0;
		}
	}
	...
	for (chgidx=0; chgidx < 2*old_nr; chgidx++)
	{
		...
		if (current_type != last_type)	{
			if (last_type != 0)	 {
				...
				// btrace(0, __LINE__, "e820 NENT ", 1, (long)new_bios[new_bios_entry].addr, (long)(new_bios[new_bios_entry].addr + new_bios[new_bios_entry].size));
